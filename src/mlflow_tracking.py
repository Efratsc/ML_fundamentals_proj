...
class MLflowTracker:
    ...
    def start_run(self, run_name: str = None):
        if mlflow.active_run() is not None:
            logger.warning("An MLflow run is already active. Ending it before starting a new run.")
            mlflow.end_run()
        if run_name is None:
    ...
    def log_confusion_matrix(self, y_true, y_pred, save_path: str = "confusion_matrix.png"):
    ...
    def log_feature_importance(self, model, feature_names, save_path: str = "feature_importance.png"):
    ...
    def register_model(self, model_name: str = "domestic_violence_model", version: str = "v1.0"):
        try:
            if self.run is None:
