...
class ModelTrainer:
    ...
    def plot_confusion_matrix(self, X_test: pd.DataFrame, y_test: pd.Series, save_path: str = None) -> None:
    ...
    def train_pipeline_with_mlflow(self) -> dict:
        ...
        try:
            ...
            confusion_matrix_path = os.path.join(self.config['paths']['metrics_dir'], 'confusion_matrix.png')
            self.mlflow_tracker.log_confusion_matrix(y_test, self.model.predict(X_test), confusion_matrix_path)
            
            # Create and log feature importance
            feature_importance_path = os.path.join(self.config['paths']['metrics_dir'], 'feature_importance.png')
            self.mlflow_tracker.log_feature_importance(self.model, X_train.columns, feature_importance_path)
