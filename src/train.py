...
class ModelTrainer:
    ...
    def load_config(self) -> dict:
        try:
            ...
            if config is None:
                logger.warning(f"Config file {self.config_path} empty, using defaults")
    ...
    def load_data(self) -> tuple:
        ...
        X_train = pd.read_csv(os.path.join(data_dir, "X_train.csv")).squeeze()
        X_test = pd.read_csv(os.path.join(data_dir, "X_test.csv")).squeeze()
    ...
    def create_model(self) -> Pipeline:
        ...
        logger.info(f"Created pipeline with TF-IDF and RandomForestClassifier (params: {model_params})")
        return self.model
    ...
    def evaluate_model(
        self, X_test: pd.DataFrame, y_test: pd.Series, X_train: pd.DataFrame = None
    ) -> dict:
    ...
    def train_pipeline_with_mlflow(self) -> dict:
        ...
        try:
            ...
            joblib.dump(
                ...
                os.path.join(self.config["paths"]["model_dir"], "model_with_features.pkl"),
            )
